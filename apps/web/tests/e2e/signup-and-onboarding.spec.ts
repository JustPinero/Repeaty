...
//
// CI runs this spec only when the manifest entry is `complete`. As of
// Request 1.5 (dashboard live), this entry is `complete`.
...
test.describe('@phase-1 signup-and-onboarding', () => {
  test('new user can sign up, complete onboarding, and reach /app', async ({ page }) => {
    ...
    // Onboarding done → dashboard greeting visible.
    await expect(page).toHaveURL(/\/app/, { timeout: 15_000 });
    await expect(page.getByRole('heading', { name: /Hi.*Ben/i })).toBeVisible();
    await expect(page.getByAltText(/Peaty/i)).toBeVisible();
  });
});
