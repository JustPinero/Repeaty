...
### `tier_change_log` (Phase 5)
Audit trail for `/admin` tier flips. Written by `flip_tier` RPC only.

| Column         | Type                              | Notes                                                |
| -------------- | --------------------------------- | ---------------------------------------------------- |
| id             | UUID PK                           |                                                      |
| actor_id       | UUID FK auth.users                | who performed the flip (caller's auth.uid())         |
| target_id      | UUID FK auth.users                | who was flipped                                      |
| from_tier      | TEXT NOT NULL                     |                                                      |
| to_tier        | TEXT NOT NULL                     | CHECK in (`free`,`pro`,`admin`)                      |
| reason         | TEXT NULL                         | free-form; ≤ 500 chars                               |
| created_at     | TIMESTAMPTZ                       |                                                      |
...
SECURITY DEFINER. Increments today's `rate_limits.count` for `(auth.uid(), p_bucket, current_date)` and returns the new count. Raises `errcode = 'P0001'` with message `RATE_LIMITED: <count> exceeded cap <cap> for bucket <bucket>` when the post-increment count exceeds `p_cap_per_day`. Raises `errcode = '42501' message = 'UNAUTHENTICATED'` when `auth.uid()` is NULL — **callers must therefore invoke this RPC under a user-context client (anon key + Authorization: Bearer <jwt>), not a service-role client**.

`GRANT EXECUTE TO authenticated`. Used by `generate-feedback` (bucket `feedback_generation`, cap 25/day) and `generate-lesson` (bucket `lesson_generation`, cap 10/day).

### `flip_tier(p_target_id uuid, p_new_tier text, p_reason text default null) → uuid` (migration `0016`)
SECURITY DEFINER. Atomically updates `profiles.tier` for `p_target_id` and inserts the corresponding `tier_change_log` row. Returns the inserted log row id. Raises:
- `UNAUTHENTICATED` (42501) when `auth.uid()` is NULL.
- `SELF_FLIP_FORBIDDEN` (42501) when `auth.uid() = p_target_id` (no self-promotion).
- `INVALID_TIER` (22023) when `p_new_tier` ∉ `{free, pro, admin}`.
- `NOT_ADMIN` (42501) when caller's `profiles.is_admin <> true`.
- `TARGET_NOT_FOUND` (02000) when `p_target_id` has no `profiles` row.
- `NO_CHANGE` (22023) when target's current tier equals `p_new_tier`.

`GRANT EXECUTE TO authenticated`. Called from the `flip-tier` Edge Function (5.2) — the Edge Function uses the user JWT so `auth.uid()` resolves correctly inside SECURITY DEFINER. **By design (single-user beta):** the function does NOT restrict `p_new_tier='admin'` — admins can elevate other users to admin.

### `insert_ai_deck_with_cards(p_owner uuid, p_language text, p_cefr text, p_deck_name text, p_cards jsonb) → uuid` (migration `0017`)
SECURITY DEFINER. Atomically inserts a `decks` row (`source = 'ai_generated'`, `owner_id = p_owner`) and its cards from a JSONB array. Returns the new `deck_id`. Raises:
- `UNAUTHENTICATED` (42501) when `auth.uid()` is NULL — **same caller rules as `bump_rate_limit`**: invoke under a user-context client.
- `OWNER_MISMATCH` (42501) when `auth.uid() <> p_owner`.
- `EMPTY_DECK` (22023) when `jsonb_array_length(p_cards) < 1`.

`GRANT EXECUTE TO authenticated`. Called from `generate-lesson`.

### `complete_onboarding(...)` (Phase 1)
Pre-existing — see migration `0007_onboarding_rpc.sql`.

### `due_cards_summary(...)` (Phase 2)
Pre-existing — used by `useDueCardsSummary` (Phase 2.4 / 2.5).
